...
- 查看单个密码按钮 + 展示全部密码开关
- 单个网站/App 可存多套用户名 + 密码 + 备注
- 拖动排序：网站/App 之间、网站内多套密码之间
- 网站支持"在浏览器中打开"（二次确认）

### AI API Key 管理（独立分区，不与密码混淆）
- 同一套增删改查，另含 **API Key** 与 **API Key 级备注**
- 单个用户下可存多套 API Key
- 三级拖动排序：条目 → 用户 → API Key
- 查看单个 / 全部密码与 Key
- 浏览器打开二次确认

### 整体
- 密码区与 API Key 区分开，信息密度适中
- 全局搜索：网站/App、各级备注、用户名、密码、API Key，支持分区筛选（全部/密码/API Key），点击跳转定位
- 列表批量选择删除
- 设置页：
  - 应用锁（默认关闭，可设安全问题恢复密码）
  - 字体大小（默认跟随系统）
...
[![Star History Chart](./assets/star-history.svg)](https://github.com/CrazyFigure/EasyPassword/stargazers)
...
not sell EasyPassword itself or offer a paid product or service whose value
derives entirely or substantially from EasyPassword without prior written
