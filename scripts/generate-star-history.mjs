...
function buildGithubHeaders(accept, token) {
  const headers = {
    Accept: accept,
    'User-Agent': 'CrazyFigure-EasyPassword-Star-History',
...
function readHistory() {
  ...
  // 允许空快照数组作为引导状态；首次 Action 运行将记录第一个观测点。
  // 同一时刻只能有一个仓库总数，重复时间戳通常意味着手工合并历史时发生了冲突。
  if (snapshots.length > 0 && new Set(snapshots.map((snapshot) => snapshot.observedAt)).size !== snapshots.length) {
